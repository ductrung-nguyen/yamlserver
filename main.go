...
func getReturnFormatFunc(c *gin.Context, format string) func(code int, obj interface{}) {
	switch format {
	case "", "json":
		return c.JSON
	case "xml":
	...
	default:
		panic(fmt.Sprintf("unsupported return format %s", format))
	}
}
...
func setUpRoute(r *gin.Engine, route Endpoint) {
	...
	methodFunc(route.Path, func(c *gin.Context) {
		...
		if matchedResponse != nil {
			getReturnFormatFunc(c, matchedResponse.ReturnFormat)(matchedResponse.ReturnCode, matchedResponse.ReturnObject)
			return
		} else {
